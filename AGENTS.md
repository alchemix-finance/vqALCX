...
- **Lazy drip:** `_drip*Bucket()` catches the bucket up to `block.timestamp` (`fulfillable = rate * elapsed`), fills FIFO entries from `head`, and dumps leftover capacity into `availableAuctionCapacity`. It runs as a side effect of state-changing vault functions; the public `drip()` advances both queues permissionlessly. Each call touches at most `MAX_DRIP_ENTRIES` (100) entries; if the queue is longer, `lastDripTime` is not advanced and the unspent budget carries over to the next call — drip gas is bounded, progress is not. Views never drip (EVM purity).
