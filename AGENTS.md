...
1. **`src/VqALCX.sol`** — the vault *and* the share token. ERC-20 + IERC4626 + ReentrancyGuard. Holds ALCX, mints/burns vqALCX. Contains zero DAO-specific logic. Governance and auctioneer are two addresses with two-step handovers (`proposeX` → `acceptX`).
...
3. **`src/VqStaking.sol`** — the DAO adapter. Users stake vqALCX here; it inherits OZ `VotesExtended` so the DAO (Aragon) reads voting power from *staked* balances. No receipt token. Push-based rewards: anyone donates reward tokens, balance-delta is lazily accrued into `rewardPerShare`.
...
- **Lazy drip:** `_drip*Bucket()` catches the bucket up to `block.timestamp` (`fulfillable = rate * elapsed`), fills FIFO entries from `head`, and dumps leftover capacity into `availableAuctionCapacity`. It runs as a side effect of state-changing vault functions; the public `drip()` advances both queues permissionlessly.
...
- `requestDeposit` **escrows ALCX at request time** (`safeTransferFrom` in `requestDeposit`). Note: `arch.md` §4.1 still claims the transfer happens at `deposit()` — the doc is stale there; the code is the truth.
...
- `mintViaAuction` (deposit side, `whenNotPaused`) mints to the winner against ALCX the auctioneer already transferred to the vault. `burnViaAuction` (no pause guard) burns from the **auctioneer's own balance** (it holds the locked winning bid) and pays the winner `payoutAmount ≤ burnAmount`; the discount stays in the vault.
- `maxDeposit`/`maxWithdraw`/`maxMint`/`maxRedeem` are **non-standard**: they return the caller/owner's total *claimable* (`filled − claimed`) across all their requests — not a liquidity cap. `deposit`/`mint`/`withdraw`/`redeem` find a single request with `claimable >= amount` via a linear scan of the user's request-ID array (gas grows with per-user request count; claims are per single request — you cannot claim across multiple requests in one call).
- Deposit auctions: higher `maxPrice` wins. Withdrawal auctions are **reverse-ranked**: a *lower* `minPrice` wins (bidder accepts a bigger discount). Bids lock assets in the auctioneer; outbid users are refunded immediately.

## Hard constraints (violating these breaks the design)

- **TC-3:** all timing uses `block.timestamp`, never `block.number` — Aragon's governance clock depends on it. `VqStaking.clock()` returns the timestamp and `CLOCK_MODE()` returns `"mode=blockstamp"`.
...
- Tests use a local `MockALCX` ERC-20 (real ALCX is not in the repo), `vm.prank`/`vm.startPrank`, and named actors: `governance = 0xCAFE`, `auctioneer = 0xBEEF`, `alice`, `bob`. Standard bucket params: `RATE = 100e18`, `CAPACITY = 10_000e18`.
...
- `arch.md` occasionally lags the code (see the `requestDeposit` escrow discrepancy above). When doc and code disagree, check `AUDIT_PLAN.md` and `audit/` notes — the audit already catalogued several doc/code deltas.
